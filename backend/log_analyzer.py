...
class PostgreSQLLogAnalyzer:
    ...
    def __init__(self, log_directory: Optional[str] = None):
        ...
        self.log_patterns = {
            ...
            'lock_timeout': re.compile(r'ERROR:\s+canceling\s+statement\s+because\s+of\s+lock\s+timeout', re.IGNORECASE)
        }
    ...
    def _generate_summary(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if results['slow_queries']:
            summary['slowest_query_duration'] = max(
                q['duration_ms'] for q in results['slow_queries']
            )
    ...
    def _generate_recommendations(self, results: Dict[str, Any]) -> List[str]:
        ...
        if results['slow_queries']:
            ...
            if slow_count > 10:
                recommendations.append(f"Обнаружено {slow_count} медленных запросов. Рекомендуется провести анализ производительности.")
        
        if results['deadlocks']:
            recommendations.append(f"Обнаружено {len(results['deadlocks'])} дедлоков. Проверьте порядок блокировок в транзакциях.")
        
        if results['lock_timeouts']:
            recommendations.append(f"Обнаружено {len(results['lock_timeouts'])} таймаутов блокировок. Рассмотрите увеличение lock_timeout.")
        
        if results['connection_issues']:
            recommendations.append(f"Обнаружено {len(results['connection_issues'])} проблем с подключениями. Проверьте настройки подключений.")
