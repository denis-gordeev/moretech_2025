...
async def get_example_queries(database_profile_id: str = None):
    ...
    try:
        ...
        if database_profile_id:
            # Получаем профиль для проверки (если не был получен выше)
            if profile is None:
                profile = profile_manager.get_profile(database_profile_id)
        ...
        # Объединяем все примеры: цепочки добавляем только для локальной дефолтной БД
        add_chains = (profile is None) or (profile and profile.name == "Default Database")
        all_examples = (test_queries + chain_examples) if add_chains else test_queries
